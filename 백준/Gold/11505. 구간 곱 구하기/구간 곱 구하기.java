...
public class Main {
	...
	private static long query(int node, int start, int end, long left, long right) {
		...
		int mid = (start + end) / 2;
		return query(node * 2, start, mid, left, right) * query(node * 2 + 1, mid + 1, end, left, right) % 1_000_000_007;
	}
	...
	private static long init(int node, int start, int end) {
		...
		int mid = (start + end) / 2;
		return tree[node] = init(node * 2, start, mid) * init(node * 2 + 1, mid + 1, end) % 1_000_000_007;
	}
